...
export default function ResponsePanel({
  ...
  if (!loading && response && !response.isError) {
    if (Array.isArray(response.body)) {
      ...
      response.status.startsWith('201')
    ) {
      recordCount = 1
    } else {
      recordCount = 0
    ...
    >
      ...
      >
        <button
          ...
          className={`sub-tab-button py-3 px-4 text-sm font-medium focus:outline-none ${activeTab === 'response-body-content'
              ? 'active'
        ...
        <button
          ...
          className={`sub-tab-button py-3 px-4 text-sm font-medium focus:outline-none ${activeTab === 'response-headers-content'
              ? 'active'
              : 'text-slate-600 hover:text-slate-800'
            }`}
        >
